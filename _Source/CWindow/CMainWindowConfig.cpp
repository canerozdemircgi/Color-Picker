...
const bool CMainWindow::ConfigLoad()
{
	QFile config(CCore::fileConfig);
	if (config.open(QIODevice::OpenModeFlag::ReadOnly))
	{
		...
		QList line(*CCore::SeparateQString(lines[++i], ' ', 2, &ok));
		CErrorReturnBool(ok)
		const int slotIndex = line[0].toInt(&ok);
		CErrorReturnBool(ok)
		CErrorReturnBool((slotIndex >= 0 && slotIndex < colorSlotSize))
		const int modeIndex = line[1].toInt(&ok);
		CErrorReturnBool(ok)
		CErrorReturnBool((modeIndex >= 0 && modeIndex < colorModeSize))
		colorSlots[slotIndex]->SetSelectedInit();
		colorModes[modeIndex]->setChecked(true);
