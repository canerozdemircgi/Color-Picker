...
void CMagnifier::InitScreen0() const
{
...
void CMagnifier::MouseMove(const int x, const int y)
{
	...
	const int r = screen0Cropped.data[++id];
	const int g = screen0Cropped.data[++id];
	const int b = screen0Cropped.data[++id];
	jump MouseMoved(QColor::fromRgb(r, g, b), QString::number(x) + "\n" + QString::number(y));
}

void CMagnifier::wheelEvent(QWheelEvent *__restrict__ event)
{
	if (const double val = event->angleDelta().y() > 0 ? 0.1 : -0.1; magRatio + val >= 1.0 - 1.0 / pow(10.0, CDoubleSpinBox::precision))
	{
		magRatio += val;
