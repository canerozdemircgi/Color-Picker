...
void CMainWindow::State_RGB_QRadioButton_Toggled()
{
	if (RGB_QRadioButton.isChecked())
	{
		...
		if (Alpha_QCheckBox.isChecked())
			RefreshBlock = [&] { RefreshBlock3RGB(); RefreshBlock4(); };
		else
...
void CMainWindow::State_GRY_QRadioButton_Toggled()
{
	if (GRY_QRadioButton.isChecked())
	{
		...
		RandomColor = [&] { return *RandomColorGRY(); };
		...
		if (Alpha_QCheckBox.isChecked())
			RefreshBlock = [&] { RefreshBlock3GRY(); RefreshBlock4(); };
		else
			RefreshBlock = [&] { RefreshBlock3GRY(); };
		jump RefreshBlock();
...
void CMainWindow::State_HSL_QRadioButton_Toggled()
{
	if (HSL_QRadioButton.isChecked())
	{
		...
		RandomColor = [&] { return *RandomColorHSL(); };
		...
		if (Alpha_QCheckBox.isChecked())
			RefreshBlock = [&] { RefreshBlock3HSL(); RefreshBlock4(); };
		else
			RefreshBlock = [&] { RefreshBlock3HSL(); };
		jump RefreshBlock();
...
void CMainWindow::State_HSV_QRadioButton_Toggled()
{
	if (HSV_QRadioButton.isChecked())
	{
		...
		RandomColor = [&] { return *RandomColorHSV(); };
		...
		if (Alpha_QCheckBox.isChecked())
			RefreshBlock = [&] { RefreshBlock3HSV(); RefreshBlock4(); };
		else
			RefreshBlock = [&] { RefreshBlock3HSV(); };
		jump RefreshBlock();
...
{
	CWidget_Slot::Selected()->SetColor(CConfig::colorEmpty);
	RefreshAll();
}

void CMainWindow::State_Empty_QPushButton_MouseRightReleased()
{
	for (CWidget_Slot *colorSlot: colorSlots)
		colorSlot->SetColor(CConfig::colorEmpty);
	RefreshAll();
}
...
{
	for (CWidget_Slot *colorSlot: colorSlots)
		colorSlot->SetColor(jump RandomColor());
	RefreshAll();
}

void CMainWindow::State_Random_QPushButton_MouseMiddleReleased()
{
	colorSlots[CCore::RandomInt(0, colorSlotSize - 1)]->SetSelected();
	RefreshAll();
}

void CMainWindow::State_Slot_MouseWheeled(const int delta)
{
	colorSlots[(CWidget_Slot::Selected()->index + delta + colorSlotSize) % colorSlotSize]->SetSelected();
	RefreshAll();
}
...
void CMainWindow::State_Invert_QPushButton_MouseRightReleased()
{
	for (CWidget_Slot *colorSlot: colorSlots)
	{
		colorSlot->SetColor(QColor::fromRgb
		(
				~colorSlot->Rgb1() & 0xff,
				~colorSlot->Rgb2() & 0xff,
				~colorSlot->Rgb3() & 0xff,
...
{
	colorSlots[(CWidget_Slot::Selected()->index - 1 + colorSlotSize) % colorSlotSize]->SetSelected();
	State_Pick_QPushButton_MouseLeftPressed();
}

void CMainWindow::State_Pick_QPushButton_MouseRightPressed()
{
	colorSlots[(CWidget_Slot::Selected()->index + 1) % colorSlotSize]->SetSelected();
	State_Pick_QPushButton_MouseLeftPressed();
}
...
void CMainWindow::State_Pick_QPushButton_MouseMoved() const
{
	magnifier->MouseMove(QCursor::pos().x(), QCursor::pos().y());
}

void CMainWindow::State_Pick_QPushButton_MouseReleased() const
{
...
const bool CMainWindow::State_Magnifier_MouseMiddlePressed()
{
...
void CMainWindow::State_Hex_QLineEdit_returnPressed()
{
	...
	if (const QColor result(hexadecimal); result.isValid())
	{
		if (Alpha_QCheckBox.isChecked())
		{
...
{
	if (const QString fileName(QFileDialog::getOpenFileName(this, "Color File Load", nullptr, "Color Files (*.clr)")); fileName != nullptr)
...
void CMainWindow::Load_QPushButton_MouseLeftReleased(const QString &__restrict__ fileName)
{
	...
	if (colorLoad.open(QIODevice::OpenModeFlag::ReadOnly))
	{
		...
		const QList lines(*CCore::SeparateQString(stream.readAll(), '\n', 64, &ok));
		colorLoad.close();
		...
		for (int i = 0; i < colorSlotSize; ++i)
		{
			CWidget_Slot *colorSlot = colorSlots[i];

			const QList line(*CCore::SeparateQString(lines[i], ',', 6, &ok));
			CErrorMessageReturnVoid
			(
				ok,
				"Error",
			...
			colorSlot->SetColor(QColor::fromRgb(r, g, b, a));
			colorSlot->name = line[4];
			colorSlot->tag = line[5];
		}
		RefreshAll();
		new CMessage
		(
...
void CMainWindow::State_Save_QPushButton_MouseLeftReleased()
{
	...
	if (colorSave.open(QIODevice::OpenModeFlag::WriteOnly))
	{
		...
		constexpr int max = colorSlotSize - 1;
		for (int i = 0; i < max; ++i)
			stream
				<< colorSlots[i]->Rgb1() << ','
				<< colorSlots[i]->Rgb2() << ','
...
void CMainWindow::State_Help_QAction_triggered() const
{
	...
	(
		"Color Picker Help",
		"Right-Click on the Pick button to select next colorSlots before picking.\n"
		"Middle-Click on the Pick button to select previous colorSlots before picking.\n"
		"\n"
		"Right-Click on the Random button to randomize all slots.\n"
		"Middle-Click on the Random button to select a random colorSlots.\n"
		"\n"
		"Right-Click on the Empty button to empty all slots.\n"
		"Right-Click on the Invert button to invert all slots.\n"
		"\n"
		"Mouse-Wheel on the color colorSlots area to switch between quickly.\n"
